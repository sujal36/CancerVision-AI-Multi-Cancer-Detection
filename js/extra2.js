...
(function guardHiddenInputObserver() {
  ...
  (function () {
    const tag = s => `[DBG ${new Date().toISOString()}] ${s}`;
    const ub = document.querySelector('.upload-block') || document.querySelector('#upload-section');
    if (!ub) { console.log(tag('no upload-block found')); return; }

    console.log(tag('debug-autohide installed'), ub);

    const styleId = 'cv-temp-hide-css';
    if (!document.getElementById(styleId)) {
      const css = `
      .upload-block [data-temp-hidden="1"],
      #upload-section [data-temp-hidden="1"] {
        visibility: hidden !important;
        opacity: 0 !important;
        pointer-events: none !important;
      }
      .upload-block.has-preview [data-temp-hidden="1"],
      #upload-section.has-preview [data-temp-hidden="1"] {
        visibility: visible !important;
        opacity: 1 !important;
        pointer-events: auto !important;
    ...
    function infoFor(n) {
      if (!n) return null;
      return { tag: n.tagName, id: n.id || null, cls: n.className || null, html: (n.outerHTML || '').slice(0, 240) };
    }

    function revealAll() {
      try {
        Array.from(ub.querySelectorAll('[data-temp-hidden="1"]')).forEach(n => {
          try { delete n.dataset.tempHidden; n.style.visibility = ''; n.style.opacity = ''; } catch (e) { }
        });
        console.log(tag('revealAll executed'));
      } catch (e) { console.warn(e); }
    }

    function tempHideNode(n) {
      try {
        if (!n || n.nodeType !== 1) return;
        n.dataset.tempHidden = '1';
        n.style.visibility = 'hidden';
        n.style.opacity = '0';
        setTimeout(() => {
          if (ub.classList.contains('has-preview')) {
            revealAll();
          } else {
            setTimeout(() => {
              if (!ub.classList.contains('has-preview')) {
                try { delete n.dataset.tempHidden; n.style.visibility = ''; n.style.opacity = ''; } catch (e) { }
                console.log(tag('temp node revealed after timeout'), infoFor(n));
              }
            }, 420);
          }
        }, 12);
      ...
      recs.forEach(r => {
        if (r.type === 'childList') {
          if (r.addedNodes && r.addedNodes.length) {
            Array.from(r.addedNodes).forEach(n => tempHideNode(n));
          }
        ...
        if (r.type === 'attributes' && r.attributeName === 'class') {
          if (ub.classList.contains('has-preview')) setTimeout(revealAll, 8);
        }
      });
    });
      ...
      recs.forEach(r => {
        if (r.type === 'childList' && r.addedNodes && r.addedNodes.length) {
          Array.from(r.addedNodes).forEach(n => {
            if (n.parentNode === ub) tempHideNode(n);
          });
        }
        if (r.type === 'attributes' && r.target === ub && r.attributeName === 'class') {
          if (ub.classList.contains('has-preview')) setTimeout(revealAll, 8);
        }
      });
    });
    mo2.observe(ub, { childList: true, attributes: true, subtree: false });

    window.__uploadAutoHide = { stop() { mo.disconnect(); mo2.disconnect(); console.log(tag('auto-hide stopped')); }, reveal: revealAll, ub };

    console.log(tag('auto-hide debug running — paste done. Now click Clear and watch console logs.'));
  })();
